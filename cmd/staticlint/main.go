// staticlint perfoms checks Go files uses multichecker.
package main
...
// ConfigFile represents path to configuration file.
const ConfFile = "config.json"

// Config represents configuration staticcheck loaded from file.
type Config struct {
	Staticcheck []string
}
...
// Use:
//
// checks all go files in the current directory and its subdirectories
// go run main.go ./...
// help
// go run main.go help
func main() {
	...
	// mylinters represents a set of linters that are enabled by default.
	// List linters:
	...
	// checkosexit  checking the use of a direct call to os.Exit in the main function of the main package
	...
	for _, v := range staticcheck.Analyzers {
		if checks[v.Analyzer.Name] {
			mylinters = append(mylinters, v.Analyzer)
		}
	}
